...
void
p_cmn_register(uint32_t pc, uint32_t code)
{
    const uint32_t Rn = (code >> 16) & 0x0f;
    const uint32_t imm5 = (code >> 7) & 0x1f;
    const uint32_t type = (code >> 5) & 0x03;
...
void
p_add_register_shifted_register(uint32_t pc, uint32_t code)
{
    const uint32_t setflags = code & (1 << 20);
    const uint32_t Rn = (code >> 16) & 0xf;
    const uint32_t Rd = (code >> 12) & 0xf;
...
void
p_eor_register_shifted_register(uint32_t pc, uint32_t code)
{
    const uint32_t setflags = code & (1 << 20);
    const uint32_t Rn = (code >> 16) & 0xf;
    const uint32_t Rd = (code >> 12) & 0xf;
...
void
p_and_register_shifted_register(uint32_t pc, uint32_t code)
{
    const uint32_t setflags = code & (1 << 20);
    const uint32_t Rn = (code >> 16) & 0xf;
    const uint32_t Rd = (code >> 12) & 0xf;
...
process_instruction(uint32_t pc)
{
    ...
    emit_code("label_%04x: __android_log_print(ANDROID_LOG_INFO, \"libfranken\", \"trace label_%04x\");", pc, pc);
    ...
    } else if ((code & 0x0ff00010) == 0x01700000) {
        p_cmn_register(pc, code);
    ...
    } else if ((code & 0x0fe00090) == 0x00800010) {
        p_add_register_shifted_register(pc, code);
    } else if ((code & 0x0fe00090) == 0x00200010) {
        p_eor_register_shifted_register(pc, code);
    } else if ((code & 0x0fe00090) == 0x00000010) {
        p_and_register_shifted_register(pc, code);
    } else {
