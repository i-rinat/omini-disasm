...
process_jump_slot_relocations(arelent *relp)
{
    ...
    if (!strcmp(ext_func_name, "__cxa_begin_cleanup")) {
        ...
        emit_code("    __android_log_print(ANDROID_LOG_DEBUG, \"libfranken\", \"        strncmp returned %d\", r0_signed);");
        emit_code("}");
        ...
        emit_code("static void func_%04x(state_t *state) {", relp->address);
        emit_code("    __android_log_print(ANDROID_LOG_DEBUG, \"libfranken\", \"calling fake sigaction(%%d, %%p, %%p)\", r0, aa(r1), aa(r2));");
